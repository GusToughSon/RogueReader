...
While $Running
	...
	; Connect to process memory if not connected
	If $hProcess = 0 Then
	...
	Sleep(10) ; Tiny delay to keep GUI responsive
WEnd
...
Func Adlib_Heal()
	If $HealerStatus = 1 And $Chat = 0 Then
		TimeToHeal()
	EndIf
EndFunc   ;==>Adlib_Heal

Func Adlib_Cure()
	If $CureStatus = 1 And $Chat = 0 Then
		CureMe()
	EndIf
EndFunc   ;==>Adlib_Cure
...
AdlibRegister("Adlib_Heal", 100)
AdlibRegister("Adlib_Cure", 100)
AdlibRegister("Adlib_Target", 100)
AdlibRegister("Adlib_Loot", 100)
