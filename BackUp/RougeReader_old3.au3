...
If $ProcessID Then
	...
	While 1
		...
		If $msg = $GUI_EVENT_CLOSE Or $msg = $ExitButton Then
			...
			Exit
		EndIf
		If $msg = ($GUI_EVENT_Minimize Or $GUI_EVENT_Maximize) Then
			Settings()
		EndIf
...
Func Settings()
	...
	Global $inputHotkey1 = GUICtrlCreateInput($hotkey1, 110, 20, 200, 20)

	; Hotkey 2 Input
	GUICtrlCreateLabel("Hotkey 2:", 10, 60, 100, 20)
	Global $inputHotkey2 = GUICtrlCreateInput($hotkey2, 110, 60, 200, 20)

	; Dropdown for variable selection
	GUICtrlCreateLabel("Select Variable:", 10, 100, 100, 20)
	Global $comboVar = GUICtrlCreateCombo("1", 110, 100, 200, 20)
	...
	; Main loop
		...
		ElseIf $msg = $btnSave Then
			; Save the inputs to global variables
			$hotkey1 = GUICtrlRead($inputHotkey1)
			$hotkey2 = GUICtrlRead($inputHotkey2)
			$selectedVar = GUICtrlRead($comboVar)
			MsgBox(0, "Configuration Saved", "Hotkey 1: " & $hotkey1 & @CRLF & "Hotkey 2: " & $hotkey2 & @CRLF & "Selected Variable: " & $selectedVar)
		EndIf
