...
#AutoIt3Wrapper_Res_Fileversion=5.0.0.63
...
Func ScanAndLootNearbyItems()
	...
	Local $origMouse = MouseGetPos()
	Local $origX = $origMouse[0]
	Local $origY = $origMouse[1]
	...
	For $i = 0 To $maxItems - 1
		...
		For $d = 0 To 8
			If $dx = $dxArr[$d] And $dy = $dyArr[$d] Then
				; Optional: Update in-game memory for cursor logic
				_WriteMemory($hProcess, $mouseXAddr, $memX[$d])
				_WriteMemory($hProcess, $mouseYAddr, $memY[$d])

				; Warp mouse, click, return
				MouseMove($clickX[$d], $clickY[$d], 0)
				MouseClick("right")
				MouseMove($origX, $origY, 0)

				ConsoleWrite(StringFormat("[StealthLoot] ?X=%d ?Y=%d (%s) ? Clicked (%d,%d)" & @CRLF, _
...
Func GUIReadMemory()
	...
	If $lastX <> -1 And $lastY <> -1 Then
		...
		If $dx > 25 Or $dy > 25 Then
			ConsoleWrite("[DeathDetect] Large movement detected: ?X=" & $dx & ", ?Y=" & $dy & ". Assuming death." & @CRLF)
